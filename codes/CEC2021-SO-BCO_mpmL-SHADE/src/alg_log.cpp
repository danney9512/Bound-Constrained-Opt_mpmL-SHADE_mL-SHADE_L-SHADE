...
Log::Log(string fileName, string func_setting, int max_FFE, int dim, int optimum, int run)
{
	max_FFE_ = max_FFE;
	problem_dim_ = dim;
	...
	string dirName = "output/" + func_setting + "/";
	if (!(filesystem::exists("output/"))) 
	{
		filesystem::create_directory("output/");
	}
	...
	{
		filesystem::create_directory(dirName);
		filesystem::copy("python/ErrorValue_result.py", dirName);
	}
	dirName += fileName;
	if (!(filesystem::exists(dirName))) 
	{
	...
	string dirK = dirName + "/" + to_string(run);
	if (!(filesystem::exists(dirK))) 
	{
		filesystem::create_directory(dirK);
	}

...
/******************************************************************************************/
//	2020/01/05 Modifiled for CEC2021 Bound Cronstrained Opt

namespace CEC2021 {

	void CEC2021_BoundConstrained_Evaluate(Individual* indiv, const CProblem& prob)
	{

		if (prob.id() == 1)				indiv->fitness() = Bent_Cigar(indiv->gene(), prob);
		else if (prob.id() == 2)		indiv->fitness() = Modified_Schwefel(indiv->gene(), prob);
		else if (prob.id() == 3)		indiv->fitness() = CEC2020::Lunacek_bi_Rastrigin(indiv->gene(), prob);
		else if (prob.id() == 4)		indiv->fitness() = Grie_Rosen(indiv->gene(), prob);
		else if (prob.id() == 5)		indiv->fitness() = CEC2020::hf01(indiv->gene(), prob);
		else if (prob.id() == 6)		indiv->fitness() = CEC2020::hf06(indiv->gene(), prob);
		else if (prob.id() == 7)		indiv->fitness() = CEC2020::hf05(indiv->gene(), prob);
		else if (prob.id() == 8)		indiv->fitness() = CEC2021::cf02(indiv->gene(), prob);
		else if (prob.id() == 9)		indiv->fitness() = CEC2021::cf04(indiv->gene(), prob);
		else if (prob.id() == 10)		indiv->fitness() = CEC2021::cf05(indiv->gene(), prob);
		else							printf("Can't find the corresponding function!\n");

		// function Bias 
		if (prob.global_optimum() != 0)
		{
			indiv->fitness() += prob.global_optimum();
		}
	}
	...
	double cf02(const Individual::GeneVec& gene_vec, const CProblem& prob) /* composition function 1 */
	{
		...
		std::vector<double> delta = { 10, 20, 30 };
		std::vector<double> bias = { 0, 100, 200 };
		if(prob.shiftable() == false) bias = { 0, 0, 0 };
	...
	double cf04(const Individual::GeneVec& gene_vec, const CProblem& prob) /* composition function 2 */
	{
		...
		std::vector<double> delta = { 10, 20, 30, 40 };
		std::vector<double> bias = { 0, 100, 200, 300 };
		if (prob.shiftable() == false) bias = { 0, 0, 0, 0 };
	...
	double cf05(const Individual::GeneVec& gene_vec, const CProblem& prob) /* composition function 3 */
	{
		...
		std::vector<double> delta = { 10, 20, 30, 40, 50 };
		std::vector<double> bias = { 0, 100, 200, 300, 400 };
		if (prob.shiftable() == false) bias = { 0, 0, 0, 0, 0 };
